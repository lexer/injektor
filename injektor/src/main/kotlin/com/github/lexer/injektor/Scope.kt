...
internal class Scope(private val name:String, private val logger: InjektorLogger) {
    private val cachedInstances: HashMap<KClass<*>, Any> = HashMap()
    private var started = false

    fun <T : Any> getOrCreateInstance(klass: KClass<T>, initializer: () -> T): T {
        if (!isStarted()) {
            logger.log(InjektorLogger.MessageType.WARNING, "Scoped instance ${klass.java.name} accessed before scope started \"${name}\"")
        }
